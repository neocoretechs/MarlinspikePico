...
* the arrays are arranged one after the other for convenience. The pins MUST share the same slice however, and represent channels A and B
* for DMA considerations and other subtelties of the Pico timer subsystem. This is enforced at the configuration level.
...
* the range that comes from the main controller. A delay is enforced between direction change to account for
* the time it takes for the H-bridge to switch direction, and to prevent damage to the motors and driver and prevent back EMF
* from sagging the bus.
* The top level abstract class AbstractMotorControl like the rest of the motor hierarchy.
...
* enable_pin - the enable pin for this channel
